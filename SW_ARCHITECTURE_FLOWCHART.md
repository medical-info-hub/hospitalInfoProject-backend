...
erDiagram
    ...
    HOSPITAL_MAIN {
        varchar hospitalCode PK "병원 코드"
        varchar hospitalName "병원명"
        varchar hospitalAddress "주소"
        varchar hospitalTel "전화번호"
        double coordinateX "경도"
        double coordinateY "위도"
        point location "PostGIS 위치"
        int totalDoctors "총 의사 수"
        varchar sidoCd "시도 코드"
        varchar sgguCd "시군구 코드"
        datetime createdAt "생성일"
        datetime updatedAt "수정일"
    }

    HOSPITAL_DETAIL {
        varchar hospitalCode PK,FK "병원 코드"
        varchar operatingHours "운영 시간"
        varchar parkingInfo "주차 정보"
        varchar closedDays "휴무일"
        varchar homepage "홈페이지"
        text facilityInfo "시설 정보"
    }

    MEDICAL_SUBJECT {
        bigint id PK "ID"
        varchar hospitalCode FK "병원 코드"
        text subjects "진료과목 목록"
        datetime createdAt "생성일"
    }
    ...
    PRO_DOC {
        bigint id PK "ID"
        varchar hospitalCode FK "병원 코드"
        varchar subjectName "진료과목명"
        int proDocCount "전문의 수"
        datetime createdAt "생성일"
    }

    PHARMACY {
        varchar pharmacyCode PK "약국 코드"
        varchar pharmacyName "약국명"
        varchar address "주소"
        varchar tel "전화번호"
        double coordinateX "경도"
        double coordinateY "위도"
    ...
    EMERGENCY_LOCATION {
        varchar dutyCode PK "응급실 코드"
        varchar dutyName "병원명"
        varchar dutyAddr "주소"
        double coordinateX "경도"
        double coordinateY "위도"
        varchar dutyTel1 "대표 전화"
        varchar dutyTel3 "응급실 전화"
        datetime createdAt "생성일"
    }

    DISEASE_STATS {
        bigint id PK "ID"
        varchar diseaseCode "질병 코드"
        varchar diseaseName "질병명"
        int year "통계 연도"
        int patientCount "환자 수"
        varchar ageGroup "연령대"
        varchar gender "성별"
        datetime createdAt "생성일"
    }

    YOUTUBE_VIDEO {
        varchar videoId PK "YouTube 비디오 ID"
        varchar title "제목"
        text description "설명"
        varchar channelTitle "채널명"
        varchar thumbnailUrl "썸네일 URL"
        datetime publishedAt "업로드 일시"
        int viewCount "조회수"
        datetime createdAt "생성일"
    }
```
