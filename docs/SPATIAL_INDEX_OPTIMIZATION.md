...
전국 약 **8만 개 병원 데이터**에서 빠르게 검색하고 트래픽이 늘어나는 상황에서 컨테이너 안정성을 위해서는 **인덱스 최적화**가 필수였습니다.
...
|------------|------|-----|------|---------|------|
| **초기 (ST_Distance만)** | ALL | NULL | 100,000 | 200-300ms | - |
| **1차 (복합 인덱스)** | ALL | NULL | 100,000 | 180-250ms | ❌ 효과 없음 |
...
EXPLAIN: type=ALL, key=NULL, rows=100000
...
EXPLAIN: type=ALL, key=NULL, rows=100000  ← 인덱스 미사용!
...
Before: 100,000행 스캔
         ↓
After:      220행 스캔 (99.78% 감소) ✅
```

```mermaid
graph LR
    A[전체 병원<br/>100,000개] -->|Full Scan<br/>200-300ms| B[결과<br/>2,834개]
